...
app.MapPost("/api/settings", async (IPhilipsTvAccess tvAccess) =>
{
    bool success = await tvAccess.SendSettings();
...
app.MapPost("/api/pause", async (IPhilipsTvAccess tvAccess) =>
{
    bool success = await tvAccess.SendPause();

    return success ? Results.Ok() : Results.StatusCode(StatusCodes.Status502BadGateway);
});

app.MapPost("/api/play", async (IPhilipsTvAccess tvAccess) =>
{
    bool success = await tvAccess.SendPlay();
